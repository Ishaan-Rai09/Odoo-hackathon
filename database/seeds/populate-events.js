...
// Import models
const Event = require('../lib/models/Event.ts').default;
const Organizer = require('../lib/models/Organizer.ts').default;
...
// Sample events
const getSampleEvents = (organizerIds) => [
  {
...
async function populateDatabase() {
  try {
    ...
    createdEvents.forEach((event, index) => {
      console.log(`${index + 1}. ${event.name} - ${event.category} - ${event.startDate.toDateString()}`);
    });
