  ...
  static async sendBookingCancellation(
    ...
    try {
      const notificationData: NotificationData = {
        ...
        customMessage: `Your booking has been cancelled. Refund of $${bookingData.refundAmount} will be processed.`
      }
      ...
      if (success) {
        ...
        console.log(`❌ Failed to send cancellation email to ${attendeeEmail}`)
      }
      
      return success
    } catch (error) {
      console.error(`Error sending cancellation notification to ${attendeeEmail}:`, error)
      return false
    }
  }
